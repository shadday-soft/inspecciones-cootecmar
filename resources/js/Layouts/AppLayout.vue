...
const sidebarNavigation = [
    ...
    { name: 'Personal', href: 'users.index', icon: UserGroupIcon, current: route().current('users.*') },
                ...
                <Dialog class="relative z-20 md:hidden" @close="mobileMenuOpen = false">
                    ...
                    <div class="fixed inset-0 z-40 flex">
                        <TransitionChild as="template" enter="transition ease-in-out duration-300 transform"
                            ...
                            <DialogPanel class="relative flex w-full max-w-xs flex-1 flex-col bg-primary pb-4 pt-5">
                                    ...
                                    <nav class="flex h-full flex-col">
                                        <div class="space-y-1">
                                            <a v-for="item in sidebarNavigation" :key="item.name" :href="item.href"
                                                :class="[item.current ? 'bg-indigo-800 text-white' : 'text-indigo-100 hover:bg-indigo-800 hover:text-white', 'group flex items-center rounded-md px-3 py-2 text-sm font-medium']"
                                                :aria-current="item.current ? 'page' : undefined">
                                                <component :is="item.icon"
                                                    :class="[item.current ? 'text-white' : 'text-indigo-300 group-hover:text-white', 'mr-3 size-6']"
                                                    aria-hidden="true" />
                                                <span>{{ item.name }}</span>
                                            </a>
                                        </div>
                                    ...
                                    <transition enter-active-class="transition ease-out duration-100"
                                        ...
                                        <MenuItems
                                            ...
                                            <div @click="item.action"
                                                :class="[active ? 'bg-gray-100 outline-none' : '', 'block px-4 py-2 text-sm text-gray-700 cursor-pointer']">
                                                {{
                                                    item.name }}</div>
                                            </MenuItem>
                                        </MenuItems>
